...
def SVCClf(numOfRun,numOfFold,C_vector,train_x,train_label,test_x,test_label,acc_measure):
    ...
    for i in range(n):
        ...
        for h in range(len(C_vector)):
            ...
            for j in range(0,k):
                ...
                nnclf = MLPClassifier()
                nnclf.fit(train_x_std,train_y)

                pred_train_y = nnclf.predict(train_x_std)
                pred_val_y = nnclf.predict(val_x_std)
    ...
    nnclf = MLPClassifier()
    nnclf.fit(tr_x_std,tr_y)

    pred_train_y = nnclf.predict(tr_x_std)
    pred_test_y = nnclf.predict(te_x_std)

    nn_acc = []
    nn_acc.append(CommonClf.clf_accuracy(tr_y,pred_train_y,'cnf'))
    nn_acc.append(CommonClf.clf_accuracy(te_y,pred_test_y,'cnf'))


    return (nn_acc,best_C,cv_val_acc,te_y,pred_test_y)
