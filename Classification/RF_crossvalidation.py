...
def RFRegression(X_val,Y_val,X_tra,Y_tra,percent,figure=False):
	reg = ensemble.RandomForestRegressor(n_estimators=20,max_features=percent)
	#reg = ensemble.RandomForestClassifier(n_estimators=10,max_features=percent)
	reg.fit(X_tra,Y_tra)
	result = reg.predict(X_tra)
	maximum = 0
	for x in range(len(result)):
		maximum = max(maximum,result[x])
	err_train = func.accuracyMeasure(Y_tra,result,0.0,'mae',maximum)
	result = reg.predict(X_val)
	err_valid = func.accuracyMeasure(Y_val,result,0.0,'mae',maximum)
...
for lamb in [0.1,0.3,0.5,0.7,0.9,1.0]: # set a loop to choose best lambda for different versions
	error = [0,0]
