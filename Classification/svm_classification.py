...
at first. Then use preprocessed data and support vector 
machine to predict the popularity of certain posts. 
...
def SVMClassification(X_val,Y_val,X_tra,Y_tra,regur,figure=False):
	classifier = svm.SVC(C=regur,kernel='linear')
...
for lamb in [1e-1,1,1e1,1e2]: # set a loop to choose best lambda for different versions
	error = [0,0]
	for times in range(repeat):
		...
		for i in range(fold):
			...
			ret = SVMClassification(std_valid_x,cross_valid_y,std_train_x,cross_train_y,lamb)
			error_valid += ret[0]
			error_train += ret[1]
...
[test_err,train_err] = SVMClassification(std_test_x,test_y,std_train_x,train_y,lamb,True)
