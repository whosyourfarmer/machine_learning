...
different depth
MAE, Hits, and AUC 3 types of error measure are utilized
...
def DT_Regressor(numOfRun,numOfFold,depth,train_Data,test_Data,err_measure,ratio):
    ...
    numOfIter = 5
    index_train = [m for m in range(numOfSamples)]
    index_test = [m for m in range(len(te_y))]
    dt_train_err = []
    dt_test_err = []
    for i in range(numOfIter):
        # Because of the hits error measure, the training/testing error should be gained in random order
        # and then be averaged
        random.shuffle(index_train)  # randomize training set at each iteration
        random.shuffle(index_test)

        tr_x_std = tr_x_std[index_train]
        tr_y = tr_y[index_train]

        te_x_std = te_x_std[index_test]
        te_y = te_y[index_test]

        pred_train_y = dt_reg.predict(tr_x_std)
        pred_test_y = dt_reg.predict(te_x_std)

        dt_train_err.append(CommonReg.errorMeasure(tr_y, pred_train_y, err_measure, ratio))
        dt_test_err.append(CommonReg.errorMeasure(te_y, pred_test_y, err_measure, ratio))

    dt_err = [np.mean(dt_train_err), np.mean(dt_test_err)]


    return (dt_err,best_depth,cv_val_err,te_y,pred_test_y)
