...
def SVR_Regressor(numOfRun,numOfFold,C_vector,train_Data,test_Data,err_measure,ratio):
    ...
    for i in range(n):
        ...
        for h in range(len(C_vector)):
            ...
            for j in range(0,k):
                ...
                svr_reg = SVR(kernel = 'linear', C = C_vector[h])
    ...
    svr_reg = SVR(kernel = 'linear', C = best_C)
    ...
    svr_err = []
    svr_err.append(CommonReg.errorMeasure(tr_y, pred_train_y,err_measure,ratio))
    svr_err.append(CommonReg.errorMeasure(te_y, pred_test_y,err_measure,ratio))
