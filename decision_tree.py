...
at first. Then use preprocessed data and decision tree (CART) 
...
def decisionTreeClassification(X_val,Y_val,X_tra,Y_tra,regur,figure=False):
	classifier = tree.DecisionTreeClassifier(max_depth=regur)
	classifier.fit(X_tra,Y_tra)
	result = classifier.predict(X_tra)
	proba = classifier.predict_proba(X_tra)
	#err_train = func.accuracyMeasure(Y_tra,result,option='precscore')
	...
	#err_valid = func.accuracyMeasure(Y_val,result,option='precscore')
...
for lamb in [3,4,5,6,7,8]: # set a loop to choose best lambda for different versions
	error = [0,0]
	for times in range(repeat):
		...
		for i in range(fold):
			...
			ret = decisionTreeClassification(std_valid_x,cross_valid_y,std_train_x,cross_train_y,lamb)
			error_valid += ret[0]
			error_train += ret[1]
...
[test_err,train_err] = decisionTreeClassification(std_test_x,test_y,std_train_x,train_y,lamb,True)
