'''
===========================================
logistic regression w.r.t l1 regularization
===========================================

This code implements standardization and cross validation
at first. Then use preprocessed data and logistic regression
with l1 regularization to predict the popularity of certain posts.  
...
def logisticRegression(X_val,Y_val,X_tra,Y_tra,regur,figure=False):
	logreg = linear_model.LogisticRegression(penalty='l1',C=regur)
	logreg.fit(X_tra,Y_tra)
	result = logreg.predict(X_tra)
	err_train = func.accuracyMeasure(Y_tra,result,option='recallscore')
	result = logreg.predict(X_val)
...
for lamb in [1,1e1,1e2,1e3]: # set a loop to choose best lambda for different versions
	error = [0,0]
	for times in range(repeat):
		...
		for i in range(fold):
			...
			ret = logisticRegression(std_valid_x,cross_valid_y,std_train_x,cross_train_y,lamb)
			error_valid += ret[0]
			error_train += ret[1]
...
[test_err,train_err] = logisticRegression(std_test_x,test_y,std_train_x,train_y,lamb,True)
